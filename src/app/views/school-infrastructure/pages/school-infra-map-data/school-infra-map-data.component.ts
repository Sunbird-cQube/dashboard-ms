...
@Component({
  selector: 'app-school-infra-map-data',
  templateUrl: './school-infra-map-data.component.html',
  styleUrls: ['./school-infra-map-data.component.scss']
})
export class SchoolInfraMapDataComponent implements OnInit {
  ...
  getReportData(values: any): void {
    ...
    Object.keys(queries).forEach(async (key: any) => {
      ...
      let query = buildQuery(onLoadQuery, defaultLevel, this.filters, this.startDate, this.endDate, key, this.compareDateRange);
